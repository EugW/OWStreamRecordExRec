...
void OWStreamRecordExRec_tick(struct OWStreamRecordExRec_data *filter, float t) {
	...
	if (width != filter->width || height != filter->height) {
		...
		filter->staging_texture = (gs_texture_t *) gs_stagesurface_create(filter->width, filter->height, GS_RGBA);
		obs_leave_graphics();
...
bool obs_module_load(void) {
    ...
    OWStreamRecordExRec.destroy = [](void *data) { OWStreamRecordExRec_destroy(reinterpret_cast<OWStreamRecordExRec_data *>(data)); };
    OWStreamRecordExRec.video_tick = [](void *data, float t) { OWStreamRecordExRec_tick(reinterpret_cast<OWStreamRecordExRec_data *>(data), t); };
    OWStreamRecordExRec.video_render = [](void *data, gs_effect_t *effect) { OWStreamRecordExRec_render(reinterpret_cast<OWStreamRecordExRec_data *>(data), effect); };
